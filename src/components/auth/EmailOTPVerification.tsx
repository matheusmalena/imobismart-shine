...
export function EmailOTPVerification({ email, onSuccess, onCancel }: EmailOTPVerificationProps) {
  ...
  const handleVerify = async () => {
    ...
    try {
      const { data, error: fnError } = await supabase.functions.invoke('verify-login-otp', {
        body: { email, code },
      });
  ...
  const handleResend = async () => {
    setIsResending(true);
    try {
      const { data, error: fnError } = await supabase.functions.invoke('send-login-otp', {
        body: { email },
      });
